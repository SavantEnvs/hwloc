/* Fleet policy (SPEC.md 6.1): disable LeakSanitizer preventively at BUILD time. ASan
   ...
   hwloc's harness drives the Base64 codec via libhwloc, which allocates topology
   state through hwloc's own allocators; those are not required to be freed on every error path, so
