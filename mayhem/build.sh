...
# Build-time LSan-off hook (SPEC.md 6.1, mayhem/lsan_off.c), linked into the libFuzzer target AND
# the standalone reproducer. Replaces a forbidden runtime ASan default-options override; ASan
# use-after-free/overflow and UBSan stay fully on and halting. Described in prose, not named:
# SPEC.md 6.2 item 15 bans the override symbol names anywhere under mayhem/, comments included.
$CC $SANITIZER_FLAGS $DEBUG_FLAGS -c "$SRC/mayhem/lsan_off.c" -o "$SRC/lsan_off.o"
...
$CXX $SANITIZER_FLAGS $DEBUG_FLAGS -o /mayhem/hwloc_fuzzer \
    "$SRC/hwloc_fuzzer.o" "$SRC/lsan_off.o" $LIB_FUZZING_ENGINE "${LINK_LIBS[@]}"
...
$CXX $SANITIZER_FLAGS $DEBUG_FLAGS -o /mayhem/hwloc_fuzzer-standalone \
    "$SRC/hwloc_fuzzer.o" "$SRC/standalone_main.o" "$SRC/lsan_off.o" "${LINK_LIBS[@]}"
